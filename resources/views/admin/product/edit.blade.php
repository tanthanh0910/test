    ...
    <div class="content user-admin-add-new">
        ...
        <div class="card mt-4 p-5 flex-1">
            ...
            <div class="d-flex justify-content-center">
                <!-- Form add new admin -->
                <form class="form-add-admin" id="formAddStore" method="post" action="{{route('products.update',$product->id)}}">
                    @csrf
                    {{ method_field('put') }}
                                ...
                                <select class="form-control" style="width: max-width" id="store_id"
                                    ...
                                    @if(!empty($stores))
                                        @foreach($stores as $item)
                                            <option value="{{$item->id}}" {{old('store_id') == $item->id || $product->store_id == $item->id ? 'selected' : '' }}>{{$item->name}}</option>
                                        @endforeach
                                    @endif
                                </select>
